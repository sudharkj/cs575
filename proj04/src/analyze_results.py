...
# simulation order
sim_order = []
# get number of different array sizes used
array_sizes = pd.unique(df['size'])
for _ in range(len(array_sizes)):
...
base_df = base_df.loc[base_df.index.repeat(len(sim_types))]
# reset index
...
results_df = results_df.sort_values(by=['size', 'type'])
