kernel void ArrayMult(global const double *dA, global const double *dB, global double *dC) {
    int gid = get_global_id(0);

    dC[gid] = dA[gid] * dB[gid];
}

kernel void ArrayMultAdd(global const double *dA, global const double *dB, global const double *dC, global double *dD) {
    int gid = get_global_id(0);

    dD[gid] = (dA[gid] * dB[gid]) + dC[gid];
}

kernel void ArrayMultReduce(global const double *dA, global const double *dB, local double *prods, global double *dC) {
