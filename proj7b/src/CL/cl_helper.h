//
// Created by sudha on 6/2/20.
//

#ifndef CS575_CL_HELPER_H
#define CS575_CL_HELPER_H
...
class CLErrorCode {
public:
    const cl_int statusCode;
    const string meaning;
};

const vector <CLErrorCode> ERROR_CODES =
        {
                {CL_SUCCESS,                         ""},
...
void printCLError(const cl_int status, const string prefix) {
    for (auto errorCode : ERROR_CODES) {
        if (status == errorCode.statusCode) {
            if (!errorCode.meaning.empty()) {
                cerr << prefix << " " << errorCode.meaning << endl;
            }
            break;
        }
    }
...
void clWait(cl_command_queue queue) {
    cl_event wait;
    cl_int status;

    status = clEnqueueMarker(queue, &wait);
    printCLError(status, "Wait: clEnqueueMarker failed");

    status = clWaitForEvents(1, &wait);
    printCLError(status, "Wait: clWaitForEvents failed");
}

#endif //CS575_CL_HELPER_H
